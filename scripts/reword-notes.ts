 ...
 */
const TERMS: readonly { before: string, after: string }[] = [
    { before: 'Vermischung', after: 'Kontamination' },
    { before: 'innheralb', after: 'innerhalb' },
    { before: 'Dennis Condon', after: 'Denis Condon' },
    { before: "Akzent auf f'", after: 'Akzent auf f′' }
]
...
TERMS.forEach(({ before, after }) => {
    const count = replaceTerm(document, before, after)
    if (count > 0) report.push(`${before} → ${after}: ${count} Stellen`)
})
