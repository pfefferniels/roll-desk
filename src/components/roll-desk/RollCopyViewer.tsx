...
import { AllAssumptions } from "./assumptions/AllAssumptions.tsx";
...
export const RollCopyViewer = ({
    ...
    useLayoutEffect(() => {
        const renderIIIF = async () => {
            ...
            if (!copy.scan) return;
            const measurement = copy.measurements.find(
                (m) => m.holeSeparation && m.margins
            );
    ...
    useEffect(() => {
        ...
        newEmulation?.emulateFromRoll(
            copy
                .getOriginalEvents()
                .filter((e) => e.type === "note" || e.type === "expression")
        );
    ...
    return (
        <>
            <g className="roll-copy" ref={svgRef}>
                ...
                />
                {[
                    ...copy.conjectures.map((c) => c.with).flat(),
                    ...copy.getOriginalEvents(),
                ].map((event) => {
                    if (
                        event.type === "note" ||
                        ...
                        event.type === "handwrittenText" ||
                        event.type === "stamp" ||
                        event.type === "rollLabel"
                    ) {
                        return (
                            <TextEvent
                                key={event.id}
...
const PerforatedEvent = ({
    ...
    const x = translateX(
        event.horizontal.from * (stretch?.factor || 1) + (shift?.horizontal || 0)
    );
    const width =
        translateX(event.horizontal.to * (stretch?.factor || 1)) -
        translateX(event.horizontal.from * (stretch?.factor || 1));

    const height = event.type === "note" ? trackHeight.note : trackHeight.expression;
    const y = trackToY(event.vertical.from + (shift?.vertical || 0) - 1) - height / 2;
    ...
    return (
        <>
            <rect
                ...
                width={width}
                height={
                    event.type === "note" ? trackHeight.note : trackHeight.expression
                }
                ...
                <>
                    ...
                    >
                            ...
                            <>
                                <tspan x={x}>
                                    ...
                                    <tspan fontWeight="bold">
                                        {event.type === "note" ? event.vertical.from : "[unknown]"}
                                    </tspan>
                                </tspan>
                                <tspan x={x} dy="1.2em">
                        ...
                        {event.type === "expression" && (
                            <tspan x={x} dy="1.2em">
                                Expression:{" "}
                                <tspan fontWeight="bold">{event.expressionType}</tspan>
                                &nbsp;(Track {event.vertical.from})
                            </tspan>
                        )}
                        <tspan x={x} dy="1.2em">
                            ...
                            {shift && stretch && (
                                <tspan>
                                    (
                                    {(
                                        (event.horizontal.from * (stretch?.factor || 1) +
                                            (shift?.horizontal || 0)) /
                                        10
                                    ).toFixed(2)}
                                    cm)
                        ...
                        <tspan x={x} dy="1.2em">
                            spans:{" "}
                            {((event.horizontal.to - event.horizontal.from) / 10).toFixed(2)}
                            cm{" "}
                            {shift && stretch && (
                                <tspan>
                                    (
                                    {(
                                        (event.horizontal.to * (stretch?.factor || 1) +
                                            (shift?.horizontal || 0) -
                                            (event.horizontal.from * (stretch?.factor || 1) +
                                                (shift?.horizontal || 0))) /
                                        10
...
const TextEvent = ({
    ...
    const x = translateX(
        horizontal.from * (stretch?.factor || 1) + (shift?.horizontal || 0)
    );
    const y = trackToY(vertical.from + (shift?.vertical || 0));

    const width = translateX(
        ((event.horizontal.to || 0) - event.horizontal.from) *
        (stretch?.factor || 1)
    );
    const height =
        trackToY(event.vertical.to || 0) - trackToY(event.vertical.from);
